 ...
 *
 * This screen is opened when X redirects back to the app after authorization.
 * It extracts the OAuth verifier from the URL, exchanges it for access tokens,
 * stores the credentials, and navigates back to the x-banner-update screen.
 */
...
import { getOAuthAccessToken } from '../../services/xOAuthFlow'
...
export default function OAuthCallbackScreen() {
  ...
  const handleCallback = async () => {
    ...
    try {
      ...
      const storedData = await AsyncStorage.getItem('x_oauth_temp')
      if (!storedData) {
        throw new Error('OAuth session not found. Please try signing in again.')
      }

      const { requestToken, requestTokenSecret } = JSON.parse(storedData)

      if (oauth_token !== requestToken) {
        throw new Error('OAuth token mismatch')
      }

      console.log('[OAuthCallback] Exchanging verifier for access token...')
      ...
      // Verify credentials
      console.log('[OAuthCallback] Verifying credentials...')
      const user = await getAuthenticatedUser(creds)
      console.log('[OAuthCallback] User verified:', user.username)

      // Store credentials
      const stored: StoredCredentials = { accessToken, accessTokenSecret, screenName }
      await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(stored))

      // Store user banner URL for the x-banner-update screen
      ...
      // Clean up temp data
      await AsyncStorage.removeItem('x_oauth_temp')
      ...
      setTimeout(() => {
        router.replace('/x-banner-update')
      }, 1000)
    } catch (err) {
      ...
      // Store error for the x-banner-update screen
      await AsyncStorage.setItem(
        'x_oauth_result',
        JSON.stringify({
          success: false,
      ...
      <View style={styles.content}>
        {status === 'processing' && (
          <View style={styles.stateContainer}>
            ...
            <SvgXml xml={scrSparkSvg} width="50" height="50" style={styles.scrSparkProcessing} />
            <SvgXml xml={scrCirclySvg} width="45" height="45" style={styles.scrCirclyProcessing} />
            <SvgXml xml={scrArrowSvg} width="40" height="40" style={styles.scrArrowProcessing} />
          </View>
        )}
        ...
        {status === 'success' && (
          <View style={styles.stateContainer}>
            <BrutalistBox
              ...
              contentStyle={styles.boxContent}
            >
              <Ionicons name="checkmark" size={18} color="#0A0A18" />
              <Text style={styles.title}>Success!</Text>
              <Text style={styles.subtitle}>Redirecting...</Text>
            </BrutalistBox>
            <SvgXml xml={scrSmileSvg} width="55" height="55" style={styles.scrSmileSuccess} />
            <SvgXml xml={scrSparkSvg} width="50" height="50" style={styles.scrSparkSuccess} />
            <SvgXml xml={scrCirclySvg} width="45" height="45" style={styles.scrCirclySuccess} />
            <SvgXml xml={scrArrowSvg} width="40" height="40" style={styles.scrArrowSuccess} />
          </View>
        )}
        ...
        {status === 'error' && (
          <View style={styles.stateContainer}>
            ...
            >
              ...
              <Text style={styles.subtitle}>Returning to previous screen...</Text>
            </BrutalistBox>
            <SvgXml xml={scrArrowSvg} width="50" height="50" style={styles.scrArrowError} />
            <SvgXml xml={scrCirclySvg} width="55" height="55" style={styles.scrCirclyError} />
            <SvgXml xml={scrSparkSvg} width="45" height="45" style={styles.scrSparkError} />
            <SvgXml xml={scrSmileSvg} width="40" height="40" style={styles.scrSmileError} />
          </View>
        )}
      </View>
    </SafeAreaView>
